...
def circBase_validator(circBase_file, circRNA_file, circRNA_ID, outfile):
    ...
    for line in circBase[1:-1]:
        ...
        list_n = [circBase_chr, circBase_start, circBase_end, circBase_ID, circBase_tissue, circBase_gene]
        circBase_list.append(list_n)

    for line in circRNA[1:-1]:
        col = line.split(',')
        circRNA_chr = col[circRNA_ID].split(":")[0]
        circRNA_start = int(col[circRNA_ID].split(":")[1].split("-")[0])
        circRNA_end = int(col[circRNA_ID].split(":")[1].split("-")[1].split(",")[0])
    ...
    for i in circBase_list:
        for j in circRNA_list:
            if i[0] == j[0] and i[2] == j[2]:
                ...
                out_list.append([i[0]+":"+str(i[1])+"-"+str(i[2]),i[3],i[4],i[5],(j[0]+":"+str(j[1])+"-"+str(j[2])),(j[1])-(i[1])])
            else:
                no_count = no_count

    outfile = open(outfile, 'w')
    outfile.write('circBase_ID' + ',' + 'circRNA_name' + ',' 'circBase_tissue' + ',' 'circBase_gene' + ','+
              'circRNA_ID' + ',' + 'Difference' + '\n')
...
circBase_validator('/Users/lachlan/Documents/Masters_Research_2019/Data/circBase/hsa_hg19_circRNA.txt','/Users/lachlan/Desktop/Diffexp.txt', 0, '/Users/lachlan/Desktop/out.csv')

# Add in test for file type. If its a csv file then ask for a tsv